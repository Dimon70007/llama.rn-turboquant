...
void quantize_row_q1_0_ref(const float * LM_GGML_RESTRICT x, block_q1_0 * LM_GGML_RESTRICT y, int64_t k) {
...
void quantize_row_mxfp4_ref(const float * LM_GGML_RESTRICT x, block_mxfp4 * LM_GGML_RESTRICT y, int64_t k) {
...
void quantize_row_nvfp4_ref(const float * LM_GGML_RESTRICT x, block_nvfp4 * LM_GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        for (int s = 0; s < n_sub; s++) {
            ...
            const uint8_t ue = lm_ggml_fp32_to_ue4m3(amax / 6.0f);
            y[i].d[s] = ue;
            const float d = lm_ggml_ue4m3_to_fp32(ue);
...
void dequantize_row_q1_0(const block_q1_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    static const int qk = QK1_0;
...
void dequantize_row_q4_0(const block_q4_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    static const int qk = QK4_0;
...
void dequantize_row_q4_1(const block_q4_1 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
...
void dequantize_row_q5_0(const block_q5_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    static const int qk = QK5_0;
...
void dequantize_row_q5_1(const block_q5_1 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
...
void dequantize_row_q8_0(const block_q8_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
...
void dequantize_row_mxfp4(const block_mxfp4 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    static const int qk = QK_MXFP4;
    ...
    for (int i = 0; i < nb; i++) {
        const float d = LM_GGML_E8M0_TO_FP32_HALF(x[i].e);
...
void dequantize_row_nvfp4(const block_nvfp4 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    static const int qk = QK_NVFP4;
    static const int qk_sub = QK_NVFP4_SUB;
    static const int n_sub = QK_NVFP4 / QK_NVFP4_SUB;
    ...
    for (int i = 0; i < nb; i++) {
        for (int s = 0; s < n_sub; s++) {
            const float d = lm_ggml_ue4m3_to_fp32(x[i].d[s]);
...
static float make_qx_quants(int n, int nmax, const float * LM_GGML_RESTRICT x, int8_t * LM_GGML_RESTRICT L, int rmse_type,
        const float * LM_GGML_RESTRICT qw) {
    float max = 0;
    float amax = 0;
    for (int i = 0; i < n; ++i) {
        float ax = fabsf(x[i]);
...
static float make_q3_quants(int n, int nmax, const float * LM_GGML_RESTRICT x, int8_t * LM_GGML_RESTRICT L, bool do_rmse) {
    float max = 0;
    float amax = 0;
    for (int i = 0; i < n; ++i) {
        float ax = fabsf(x[i]);
...
static float make_qkx1_quants(int n, int nmax, const float * LM_GGML_RESTRICT x, uint8_t * LM_GGML_RESTRICT L, float * LM_GGML_RESTRICT the_min,
...
static float make_qkx2_quants(int n, int nmax, const float * LM_GGML_RESTRICT x, const float * LM_GGML_RESTRICT weights,
        uint8_t * LM_GGML_RESTRICT L, float * LM_GGML_RESTRICT the_min, uint8_t * LM_GGML_RESTRICT Laux,
...
static inline void get_scale_min_k4(int j, const uint8_t * LM_GGML_RESTRICT q, uint8_t * LM_GGML_RESTRICT d, uint8_t * LM_GGML_RESTRICT m) {
    if (j < 4) {
        *d = q[j] & 63; *m = q[j + 4] & 63;
    } else {
...
void dequantize_row_q2_K(const block_q2_K * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
...
static float make_qkx3_quants(int n, int nmax, const float * LM_GGML_RESTRICT x, const float * LM_GGML_RESTRICT weights,
        uint8_t * LM_GGML_RESTRICT L, float * LM_GGML_RESTRICT the_min, uint8_t * LM_GGML_RESTRICT Laux,
        float rmin, float rdelta, int nstep, bool use_mad) {
    float min = x[0];
    float max = x[0];
...
static float make_qp_quants(int n, int nmax, const float * LM_GGML_RESTRICT x, uint8_t * LM_GGML_RESTRICT L, const float * quant_weights) {
    float max = 0;
    for (int i = 0; i < n; ++i) {
        max = MAX(max, x[i]);
    }
...
static void quantize_row_q2_K_impl(const float * LM_GGML_RESTRICT x, block_q2_K * LM_GGML_RESTRICT y, int k, const float * LM_GGML_RESTRICT quant_weights) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < QK_K/16; ++j) {
            const float * LM_GGML_RESTRICT qw = quant_weights + QK_K * i + 16*j;
...
size_t quantize_q2_K(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
void dequantize_row_q3_K(const block_q3_K * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        const uint8_t * LM_GGML_RESTRICT q = x[i].qs;
...
static void quantize_row_q3_K_impl(const float * LM_GGML_RESTRICT x, block_q3_K * LM_GGML_RESTRICT y, int64_t n_per_row, const float * LM_GGML_RESTRICT quant_weights) {
...
size_t quantize_q3_K(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
void dequantize_row_q4_K(const block_q4_K * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int nb = k / QK_K;
...
static void quantize_row_q4_K_impl(const float * LM_GGML_RESTRICT x, block_q4_K * LM_GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
    assert(n_per_row % QK_K == 0);
...
size_t quantize_q4_K(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
void quantize_row_q5_K_ref(const float * LM_GGML_RESTRICT x, block_q5_K * LM_GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        uint8_t * LM_GGML_RESTRICT qh = y[i].qh;
        uint8_t * LM_GGML_RESTRICT ql = y[i].qs;
        memset(qh, 0, QK_K/8);
...
void dequantize_row_q5_K(const block_q5_K * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
...
static void quantize_row_q5_K_impl(const float * LM_GGML_RESTRICT x, block_q5_K * LM_GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
    assert(n_per_row % QK_K == 0);
    const int64_t nb = n_per_row / QK_K;
...
size_t quantize_q5_K(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
void quantize_row_q6_K_ref(const float * LM_GGML_RESTRICT x, block_q6_K * LM_GGML_RESTRICT y, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        uint8_t * LM_GGML_RESTRICT ql = y[i].ql;
        uint8_t * LM_GGML_RESTRICT qh = y[i].qh;
        for (int j = 0; j < QK_K; j += 128) {
            for (int l = 0; l < 32; ++l) {
...
void dequantize_row_q6_K(const block_q6_K * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        const uint8_t * LM_GGML_RESTRICT ql = x[i].ql;
        const uint8_t * LM_GGML_RESTRICT qh = x[i].qh;
        const int8_t  * LM_GGML_RESTRICT sc = x[i].scales;
...
static void quantize_row_q6_K_impl(const float * LM_GGML_RESTRICT x, block_q6_K * LM_GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
    assert(n_per_row % QK_K == 0);
    const int64_t nb = n_per_row / QK_K;
...
size_t quantize_q6_K(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
static void quantize_row_q4_0_impl(const float * LM_GGML_RESTRICT x, block_q4_0 * LM_GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
...
size_t quantize_q1_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q1_0_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * lm_ggml_row_size(LM_GGML_TYPE_Q1_0, n_per_row);
    }
    size_t row_size = lm_ggml_row_size(LM_GGML_TYPE_Q1_0, n_per_row);
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
...
size_t quantize_q4_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
...
static void quantize_row_q4_1_impl(const float * LM_GGML_RESTRICT x, block_q4_1 * LM_GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
...
size_t quantize_q4_1(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
...
static void quantize_row_q5_0_impl(const float * LM_GGML_RESTRICT x, block_q5_0 * LM_GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
...
size_t quantize_q5_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
...
static void quantize_row_q5_1_impl(const float * LM_GGML_RESTRICT x, block_q5_1 * LM_GGML_RESTRICT y, int64_t n_per_row, const float * quant_weights) {
...
size_t quantize_q5_1(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
...
size_t quantize_q8_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
size_t quantize_mxfp4(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_UNUSED(quant_weights);
    quantize_row_mxfp4_ref(src, dst, (int64_t)nrow*n_per_row);
    return nrow * lm_ggml_row_size(LM_GGML_TYPE_MXFP4, n_per_row);
}

size_t quantize_nvfp4(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_UNUSED(quant_weights);
    quantize_row_nvfp4_ref(src, dst, (int64_t)nrow*n_per_row);
    return nrow * lm_ggml_row_size(LM_GGML_TYPE_NVFP4, n_per_row);
}

// ====================== Ternary (de)-quantization (BitNet b1.58 and TriLMs)

void quantize_row_tq1_0_ref(const float * LM_GGML_RESTRICT x, block_tq1_0 * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
...
void quantize_row_tq2_0_ref(const float * LM_GGML_RESTRICT x, block_tq2_0 * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
...
size_t quantize_tq1_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
...
size_t quantize_tq2_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
...
void dequantize_row_tq1_0(const block_tq1_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
...
void dequantize_row_tq2_0(const block_tq2_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
...
void dequantize_row_q8_K(const block_q8_K * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_K == 0);
    const int64_t nb = k / QK_K;
...
static int iq2_find_best_neighbour(const uint16_t * LM_GGML_RESTRICT neighbours, const uint64_t * LM_GGML_RESTRICT grid,
        const float * LM_GGML_RESTRICT xval, const float * LM_GGML_RESTRICT weight, float scale, int8_t * LM_GGML_RESTRICT L) {
...
static void quantize_row_iq2_xxs_impl(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t n, const float * LM_GGML_RESTRICT quant_weights) {
...
static void quantize_row_iq2_xs_impl(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t n, const float * LM_GGML_RESTRICT quant_weights) {
...
size_t quantize_iq2_xxs(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
size_t quantize_iq2_xs(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_ASSERT(n_per_row%QK_K == 0);
    int64_t nblock = n_per_row/QK_K;
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
...
void lm_iq3xs_init_impl(int grid_size) {
    ...
    // See explanation of parallelism in lm_iq2xs_init_impl
    int * n_per_i = (int *)malloc(kmap_size*sizeof(int));
    LM_GGML_ASSERT(n_per_i);
    int num_neighbors = 0, num_not_in_map = 0;
#ifdef LM_GGML_USE_OPENMP
...
static int iq3_find_best_neighbour(const uint16_t * LM_GGML_RESTRICT neighbours, const uint32_t * LM_GGML_RESTRICT grid,
        const float * LM_GGML_RESTRICT xval, const float * LM_GGML_RESTRICT weight, float scale, int8_t * LM_GGML_RESTRICT L) {
    int num_neighbors = neighbours[0];
    LM_GGML_ASSERT(num_neighbors > 0);
    float best_d2 = FLT_MAX;
...
static void quantize_row_iq3_xxs_impl(int grid_size, const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t n,
        const float * LM_GGML_RESTRICT quant_weights) {
...
size_t quantize_iq3_xxs(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_ASSERT(n_per_row%QK_K == 0);
    int64_t nblock = n_per_row/QK_K;
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
...
static void quantize_row_iq3_s_impl(int block_size, const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int n,
        const float * LM_GGML_RESTRICT quant_weights,
        float   * scales,
        float   * weight,
        float   * xval,
...
size_t quantize_iq3_s(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_ASSERT(n_per_row%QK_K == 0);
    int64_t nblock = n_per_row/QK_K;
...
static int iq1_find_best_neighbour(const uint16_t * LM_GGML_RESTRICT neighbours, const uint64_t * LM_GGML_RESTRICT grid,
        const float * LM_GGML_RESTRICT xval, const float * LM_GGML_RESTRICT weight, float * scale, int8_t * LM_GGML_RESTRICT L, int ngrid) {
    int num_neighbors = neighbours[0];
    LM_GGML_ASSERT(num_neighbors > 0);
...
static int iq1_find_best_neighbour2(const uint16_t * LM_GGML_RESTRICT neighbours, const uint64_t * LM_GGML_RESTRICT grid,
        const float * LM_GGML_RESTRICT xval, const float * LM_GGML_RESTRICT weight, float scale, const float * LM_GGML_RESTRICT xg, int8_t * LM_GGML_RESTRICT L, int ngrid) {
    int num_neighbors = neighbours[0];
    LM_GGML_ASSERT(num_neighbors > 0);
    float best_score = FLT_MAX;
    int grid_index = -1;
...
static void quantize_row_iq1_s_impl(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t n, const float * LM_GGML_RESTRICT quant_weights,
        float    * scales,
        float    * weight,
        float    * sumx,
        float    * sumw,
...
size_t quantize_iq1_s(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_ASSERT(n_per_row%QK_K == 0);
...
static void quantize_row_iq1_m_impl(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t n, const float * LM_GGML_RESTRICT quant_weights,
        float    * scales,
        float    * weight,
        float    * pairs,
        int8_t   * L,
...
size_t quantize_iq1_m(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_ASSERT(n_per_row%QK_K == 0);
...
static void quantize_row_iq4_nl_impl(const int super_block_size, const int block_size, const float * LM_GGML_RESTRICT x,
...
size_t quantize_iq4_nl(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
...
//void quantize_row_iq4_nl_ref(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t k) {
...
size_t quantize_iq4_xs(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_ASSERT(n_per_row%QK_K == 0);
    int64_t nblock = n_per_row/QK_K;
    char * qrow = (char *)dst;
    uint8_t L[QK_K];
...
static void quantize_row_iq2_s_impl(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t n, const float * LM_GGML_RESTRICT quant_weights) {
...
size_t quantize_iq2_s(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    LM_GGML_ASSERT(n_per_row%QK_K == 0);
    int64_t nblock = n_per_row/QK_K;
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
...
static bool validate_e_e8m0(uint8_t e, size_t i) {
    if (e == 0xff) {
        fprintf(stderr, "lm_ggml_validate_row_data: found invalid e value %d at block %zu\n", e, i);
        return false;
    }

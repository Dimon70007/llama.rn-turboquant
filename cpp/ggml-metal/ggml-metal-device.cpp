...
lm_ggml_metal_pipeline_with_params lm_ggml_metal_library_get_pipeline_turbo_wht(lm_ggml_metal_library_t lib) {
    ...
    if (!res.pipeline) {
        ...
        lm_ggml_metal_cv_t cv = lm_ggml_metal_cv_init();
        res = lm_ggml_metal_library_compile_pipeline(lib, name, name, cv);
        lm_ggml_metal_cv_free(cv);
    }
...
lm_ggml_metal_pipeline_with_params lm_ggml_metal_library_get_pipeline_mul_mm_tq_rotated(lm_ggml_metal_library_t lib, const lm_ggml_tensor * op) {
    char base[256];
    char name[256];
    ...
    snprintf(base, 256, "kernel_mul_mm_%s_rotated_%s", lm_ggml_type_name(tsrc0), lm_ggml_type_name(tsrc1));
    snprintf(name, 256, "%s_bci=%d_bco=%d_ne12=%d_ne13=%d_r2=%d_r3=%d",
             base, bc_inp, bc_out, ne12, ne13, r2, r3);
...
lm_ggml_metal_pipeline_with_params lm_ggml_metal_library_get_pipeline_mul_mm_id_tq_rotated(lm_ggml_metal_library_t lib, const lm_ggml_tensor * op) {
    char base[256];
    char name[256];
    ...
    snprintf(base, 256, "kernel_mul_mm_id_%s_rotated_%s", lm_ggml_type_name(tsrc0), lm_ggml_type_name(tsrc1));
...
lm_ggml_metal_pipeline_with_params lm_ggml_metal_library_get_pipeline_tq3_rotate_act(lm_ggml_metal_library_t lib, bool inverse) {
    char name[256];
...
lm_ggml_metal_pipeline_with_params lm_ggml_metal_library_get_pipeline_flash_attn_ext(
    ...
    snprintf(base, 256, "kernel_%s_k%s_v%s_dk%d_dv%d",
            ...
            lm_ggml_type_name(op->src[2]->type),
            dk,
            dv);

...
// emscripten uses max_align_t == 8, so we need LM_GGML_MEM_ALIGN == 8 for 64-bit wasm.
// (for 32-bit wasm, the first conditional is true and LM_GGML_MEM_ALIGN stays 4.)
// ref: https://github.com/ggml-org/llama.cpp/pull/18628
    #define LM_GGML_MEM_ALIGN 8
#else
...
#define LM_GGML_ROPE_TYPE_IMROPE 40 // binary: 101000
...
extern "C" {
    ...
    enum lm_ggml_type {
        ...
        LM_GGML_TYPE_Q1_0    = 41,
        LM_GGML_TYPE_TURBO2_0 = 42, // TurboQuant 2-bit KV cache: WHT + 2-bit PolarQuant
        LM_GGML_TYPE_TURBO3_0 = 43, // TurboQuant 3-bit KV cache: WHT + 3-bit PolarQuant
        LM_GGML_TYPE_TURBO4_0 = 44, // TurboQuant 4-bit KV cache: WHT + 4-bit PolarQuant
        LM_GGML_TYPE_TQ3_1S  = 45, // TurboQuant 3-bit weight: WHT-rotated 8-level Lloyd-Max, block_size=32
        LM_GGML_TYPE_TQ4_1S  = 46, // TurboQuant 4-bit weight: WHT-rotated 16-level Lloyd-Max, block_size=32
        LM_GGML_TYPE_COUNT   = 47,
    };
    ...
    enum lm_ggml_ftype {
        ...
        LM_GGML_FTYPE_MOSTLY_Q1_0    = 27, // except 1d tensors
    };
    ...
    LM_GGML_API struct lm_ggml_tensor * lm_ggml_im2col_3d(
            struct lm_ggml_context * ctx,
            struct lm_ggml_tensor  * a,
            struct lm_ggml_tensor  * b,
            int64_t               IC,
    ...
    LM_GGML_API struct lm_ggml_tensor * lm_ggml_conv_3d_direct(
            struct lm_ggml_context * ctx,
    ...
    // similar to lm_ggml_top_k but implemented as `argsort` + `view`
    LM_GGML_API struct lm_ggml_tensor * lm_ggml_argsort_top_k(
            struct lm_ggml_context * ctx,
            struct lm_ggml_tensor  * a,
            int                   k);
    ...
    */
    ...
    LM_GGML_API struct lm_ggml_tensor * lm_ggml_turbo_wht(
            ...
            struct lm_ggml_tensor  * scale);        // NULL = no InnerQ scaling

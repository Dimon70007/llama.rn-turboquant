...
void llama_context::sched_reserve() {
    ...
    if (cparams.auto_fgdn) {
        ...
        if (cparams.fused_gdn_ch) {
            ...
            // the lm_ggml_mul_mat assertion fails. this matches the pp reservation below (line ~553).
...
std::map<lm_ggml_backend_buffer_type_t, llama_memory_breakdown_data> llama_context::memory_breakdown() const {
...
llama_context * llama_init_from_model(
    ...
    if (params.flash_attn_type == LLAMA_FLASH_ATTN_TYPE_DISABLED &&
        (params.type_k == LM_GGML_TYPE_TURBO3_0 || params.type_k == LM_GGML_TYPE_TURBO4_0 ||
         params.type_v == LM_GGML_TYPE_TURBO3_0 || params.type_v == LM_GGML_TYPE_TURBO4_0)) {
...
void llama_memory_breakdown_print(const struct llama_context * ctx) {
    const std::vector<lm_ggml_backend_dev_t> & devices = ctx->get_model().devices;

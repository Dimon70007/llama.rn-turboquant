...
static constexpr int      LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS = 2;
static constexpr uint32_t LM_GGML_KLEIDIAI_PACK_MAGIC       = 0x4b4c4149; // "KLAI"
static constexpr uint16_t LM_GGML_KLEIDIAI_PACK_VERSION     = 1;
static constexpr size_t   LM_GGML_KLEIDIAI_PACK_ALIGN       = 64;

struct lm_ggml_kleidiai_context {
    cpu_feature features;
    lm_ggml_kleidiai_kernels * kernels_q4;
    lm_ggml_kleidiai_kernels * kernels_q8;
...
static size_t detect_num_smcus() {
    ...
    // table for known M4 variants. Users can override via LM_GGML_KLEIDIAI_SME=<n>.
    char chip_name[256] = {};
...
static int parse_uint_env(const char *s, const char *name, bool *ok) {
    ...
    if (end == s || *end != '\0') {
        LM_GGML_LOG_WARN("kleidiai: invalid %s='%s' (expected integer)\n", name, s);
        *ok = false;
        return 0;
    }
    if (v < 0 || v > INT_MAX) {
        LM_GGML_LOG_WARN("kleidiai: out-of-range %s='%s'\n", name, s);
        *ok = false;
        return 0;
    }
    *ok = true;
...
static void init_kleidiai_context(void) {
    ...
    if (!initialized) {
        ...
        const char *env_sme         = getenv("LM_GGML_KLEIDIAI_SME");
        const char *env_threads     = getenv("LM_GGML_TOTAL_THREADS");
        const char *env_chunk_mult  = getenv("LM_GGML_KLEIDIAI_CHUNK_MULTIPLIER");

        const bool cpu_has_sme = lm_ggml_cpu_has_sme();
        size_t detected_smcus = 0;

        ctx.features  = (lm_ggml_cpu_has_dotprod()     ? CPU_FEATURE_DOTPROD : CPU_FEATURE_NONE) |
                        (lm_ggml_cpu_has_matmul_int8() ? CPU_FEATURE_I8MM    : CPU_FEATURE_NONE) |
                        ((lm_ggml_cpu_has_sve() && lm_ggml_cpu_get_sve_cnt() == QK8_0) ? CPU_FEATURE_SVE : CPU_FEATURE_NONE);

        if (env_threads) {
            bool ok = false;
            int hint = parse_uint_env(env_threads, "LM_GGML_TOTAL_THREADS", &ok);
            if (ok && hint > 0) {
        ...
        if (env_chunk_mult) {
            bool ok = false;
            int multiplier = parse_uint_env(env_chunk_mult, "LM_GGML_KLEIDIAI_CHUNK_MULTIPLIER", &ok);
            if (ok && multiplier > 0) {
        ...
        if (!cpu_has_sme) {
            if (sme_env_set) {
                bool ok = false;
                int req = parse_uint_env(env_sme, "LM_GGML_KLEIDIAI_SME", &ok);
                if (ok && req > 0) {
                    LM_GGML_LOG_WARN("kleidiai: LM_GGML_KLEIDIAI_SME=%d but SME is not supported on this CPU; disabling SME\n", req);
                }
            }
            sme_cores = 0;
        } else {
            if (sme_env_set) {
                bool ok = false;
                int v = parse_uint_env(env_sme, "LM_GGML_KLEIDIAI_SME", &ok);
                sme_env_ok = ok;

                if (!ok) {
                    LM_GGML_LOG_WARN("kleidiai: LM_GGML_KLEIDIAI_SME set but parsing failed; falling back to runtime SME-core detection\n");
            ...
            if (!sme_env_set && sme_cores == 0) {
                LM_GGML_LOG_WARN("kleidiai: SME supported but runtime SME-core detection returned 0; falling back to NEON\n");
            }
        ...
        // Kernel selection
        ctx.kernels_q4 = lm_ggml_kleidiai_select_kernels_q4_0(ctx.features);
        ctx.kernels_q8 = lm_ggml_kleidiai_select_kernels_q8_0(ctx.features);

        if (!ctx.kernels_q4) {
            LM_GGML_LOG_INFO("kleidiai: no compatible q4 kernels found for CPU features mask %d\n", (int)ctx.features);
        } else {
            LM_GGML_LOG_INFO("kleidiai: primary q4 kernel feature %s\n", cpu_feature_to_string(ctx.kernels_q4->required_cpu));
        }

        if (!ctx.kernels_q8) {
            LM_GGML_LOG_INFO("kleidiai: no compatible q8 kernels found for CPU features mask %d\n", (int)ctx.features);
        } else {
            LM_GGML_LOG_INFO("kleidiai: primary q8 kernel feature %s\n", cpu_feature_to_string(ctx.kernels_q8->required_cpu));
        }
        ...
        if (ctx.features & CPU_FEATURE_SME) {
            if (sme_env_set && sme_env_ok && sme_cores > 0) {
                LM_GGML_LOG_INFO("kleidiai: SME enabled (LM_GGML_KLEIDIAI_SME=%d override)\n", sme_cores);
            } else {
                LM_GGML_LOG_INFO("kleidiai: SME enabled (runtime-detected SME cores=%d)\n", sme_cores);
            }
        } else {
            LM_GGML_LOG_INFO("kleidiai: SME disabled\n");
        }
    }
...
static inline kleidiai_block_args kleidiai_get_block_args(lm_ggml_type rhs_type) {
    switch (rhs_type) {
...
struct kleidiai_weight_header {
    ...
    uint16_t version;
    uint16_t slot_count;
    uint64_t offsets[LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS];
    uint64_t sizes[LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS];
};
...
static inline bool kleidiai_is_weight_header_valid(const kleidiai_weight_header * header) {
    ...
    if (header->magic != LM_GGML_KLEIDIAI_PACK_MAGIC || header->version != LM_GGML_KLEIDIAI_PACK_VERSION) {
        return false;
    }
    if (header->slot_count == 0 || header->slot_count > LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS) {
        return false;
    }
    return true;
}
...
static inline lm_ggml_kleidiai_kernels * kleidiai_primary_kernel_q4() {
    return ctx.kernels_q4;
}

static inline lm_ggml_kleidiai_kernels * kleidiai_primary_kernel_q8() {
    return ctx.kernels_q8;
}

template <typename SelectFallback>
static int kleidiai_collect_kernel_chain_common(
        lm_ggml_kleidiai_kernels * primary,
        cpu_feature features,
        std::array<lm_ggml_kleidiai_kernels *, LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS> & out,
    ...
    if ((primary->required_cpu & CPU_FEATURE_SME) == CPU_FEATURE_SME) {
        ...
        if (fallback_mask != CPU_FEATURE_NONE) {
            lm_ggml_kleidiai_kernels * fallback = select_fallback(fallback_mask);
...
static int kleidiai_collect_kernel_chain(const struct lm_ggml_tensor * op,
        std::array<lm_ggml_kleidiai_kernels *, LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS> & out) {
    lm_ggml_kleidiai_kernels * primary = lm_ggml_kleidiai_select_kernels(ctx.features, op);
    return kleidiai_collect_kernel_chain_common(primary, ctx.features, out,
        [&](cpu_feature mask) { return lm_ggml_kleidiai_select_kernels(mask, op); });
}

static int kleidiai_collect_q4_chain(std::array<lm_ggml_kleidiai_kernels *, LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS> & out) {
    lm_ggml_kleidiai_kernels * primary = kleidiai_primary_kernel_q4();
    return kleidiai_collect_kernel_chain_common(primary, ctx.features, out,
        [&](cpu_feature mask) { return lm_ggml_kleidiai_select_kernels_q4_0(mask); });
}

static int kleidiai_collect_q8_chain(std::array<lm_ggml_kleidiai_kernels *, LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS> & out) {
    lm_ggml_kleidiai_kernels * primary = kleidiai_primary_kernel_q8();
    return kleidiai_collect_kernel_chain_common(primary, ctx.features, out,
        [&](cpu_feature mask) { return lm_ggml_kleidiai_select_kernels_q8_0(mask); });
}

static inline int64_t lm_ggml_ne(const lm_ggml_tensor * tensor, int dim) {
...
class tensor_traits : public ggml::cpu::tensor_traits {
    bool work_size(int /* n_threads */, const struct lm_ggml_tensor * op, size_t & size) override {
        ...
        std::array<lm_ggml_kleidiai_kernels *, LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS> kernel_chain;
        ...
        if (op->src[0]->type == LM_GGML_TYPE_Q4_0 || op->src[0]->type == LM_GGML_TYPE_Q8_0) {
            const size_t qk = (op->src[0]->type == LM_GGML_TYPE_Q4_0) ? QK4_0 : QK8_0;

            size_t cursor = 0;
            bool any_slot = false;

            for (int slot = 0; slot < slot_count; ++slot) {
                lm_ggml_kleidiai_kernels * kernels = kernel_chain[slot];
                ...
                cursor = align_up(cursor, LM_GGML_KLEIDIAI_PACK_ALIGN);
                cursor += packed;
                any_slot = true;
            }
        ...
        if (op->src[0]->type == LM_GGML_TYPE_F16) {
            ...
            LM_GGML_ASSERT(rhs_batch_size0 > 0);
    ...
    bool compute_forward(struct lm_ggml_compute_params * params, struct lm_ggml_tensor * dst) override {
        if (dst->op == LM_GGML_OP_MUL_MAT) {
            if (dst->src[0]->type == LM_GGML_TYPE_Q4_0 || dst->src[0]->type == LM_GGML_TYPE_Q8_0) {
                return compute_forward_qx(params, dst);
            } else if (dst->src[0]->type == LM_GGML_TYPE_F16) {
                return compute_forward_fp16(params, dst);
            }
        } else if (dst->op == LM_GGML_OP_GET_ROWS) {
            if (dst->src[0]->type == LM_GGML_TYPE_Q4_0 || dst->src[0]->type == LM_GGML_TYPE_Q8_0) {
    ...
    bool compute_forward_fp16(lm_ggml_compute_params * params, struct lm_ggml_tensor * dst) {
        const lm_ggml_tensor * src0 = dst->src[0];
        const lm_ggml_tensor * src1 = dst->src[1];

        LM_GGML_TENSOR_BINARY_OP_LOCALS

        lm_ggml_kleidiai_kernels *kernels = lm_ggml_kleidiai_select_kernels(ctx.features, dst);
        if (!kernels) {
            return false;
        }
        ...
        LM_GGML_ASSERT(rhs_batch_size0 > 0);
        LM_GGML_ASSERT(lhs_batch_size0 % rhs_batch_size0 == 0);
        const int64_t r = lhs_batch_size0 / rhs_batch_size0;
        ...
        LM_GGML_ASSERT(wsize_required <= params->wsize);
    ...
    bool compute_forward_qx(struct lm_ggml_compute_params * params, struct lm_ggml_tensor * dst) {
        LM_GGML_ASSERT(dst->src[0]->type == LM_GGML_TYPE_Q4_0 || dst->src[0]->type == LM_GGML_TYPE_Q8_0);
        ...
        struct runtime_slot {
            int slot_index;
            lm_ggml_kleidiai_kernels * kernels;
            kernel_info * kernel;
        ...
        std::array<runtime_slot, LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS> runtime{};
        int runtime_count = 0;

        for (int slot = 0; slot < slot_total && runtime_count < LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS; ++slot) {
            lm_ggml_kleidiai_kernels * kernels = kernel_chain[slot];
        ...
        if (runtime_count == 0) {
            LM_GGML_LOG_WARN("kleidiai: no runtime kernel slot available for supported op %s\n", dst->name);
            return false;
        }
        ...
        int fallback_indices[LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS];
        int fallback_count = 0;
        // The current hybrid chain is bounded to SME + one non-SME fallback slot.
        LM_GGML_ASSERT(LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS == 2);
        for (int i = 0; i < runtime_count; ++i) {
            if (i == sme_slot) {
                continue;
            }
        ...
        LM_GGML_ASSERT(cursor <= params->wsize);
        ...
        LM_GGML_ASSERT(nchunk_size <= (size_t)INT_MAX);
        ...
        for (int64_t batch_idx = 0; batch_idx < ne12; ++batch_idx) {
            ...
            if (ith_total == 0) {
                lm_ggml_threadpool_chunk_set(params->threadpool, nth_total);
            }
    ...
    bool compute_forward_get_rows(struct lm_ggml_compute_params * params, struct lm_ggml_tensor * dst) {
        LM_GGML_ASSERT(dst->src[0]->type == LM_GGML_TYPE_Q4_0 || dst->src[0]->type == LM_GGML_TYPE_Q8_0);
        const lm_ggml_tensor * src0 = dst->src[0];
        const lm_ggml_tensor * src1 = dst->src[1];
        ...
        const lm_ggml_type rhs_type = kernels->rhs_type;
        size_t block_len = 0;
        size_t num_bytes_multiplier = 0;
        if (rhs_type == LM_GGML_TYPE_Q4_0) {
            block_len = QK4_0;
            num_bytes_multiplier = sizeof(uint16_t);
        } else if (rhs_type == LM_GGML_TYPE_Q8_0) {
            block_len = QK8_0;
        ...
        for (int64_t i = ir0; i < ir1; ++i) {
            ...
            LM_GGML_ASSERT(row_idx >= 0 && row_idx < src0->ne[1]);
...
public:
    int repack(struct lm_ggml_tensor * tensor, const void * data, size_t data_size) {
        LM_GGML_ASSERT(tensor->type == LM_GGML_TYPE_Q4_0 || tensor->type == LM_GGML_TYPE_Q8_0);
        ...
        header->magic      = LM_GGML_KLEIDIAI_PACK_MAGIC;
        header->version    = LM_GGML_KLEIDIAI_PACK_VERSION;
        header->slot_count = 0;
        ...
        std::array<lm_ggml_kleidiai_kernels *, LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS> kernel_chain;
        const bool want_q8 = tensor->type == LM_GGML_TYPE_Q8_0;
        ...
        if (want_q8 && slot_total > 0) {
            ...
            for (size_t row = 0; row < n; ++row) {
                ...
                for (size_t block = 0; block < k_blocks; ++block) {
                    ...
                    const float d = LM_GGML_FP16_TO_FP32(blk.d);
        ...
        for (int slot = 0; slot < slot_total && slot < LM_GGML_KLEIDIAI_MAX_KERNEL_SLOTS; ++slot) {
            ...
            const size_t sr = kernel->get_sr();
            const lm_ggml_type rhs_type = kernels->rhs_type;
            const size_t block_len = rhs_type == LM_GGML_TYPE_Q8_0 ? QK8_0 :
                                     rhs_type == LM_GGML_TYPE_Q4_0 ? QK4_0 : 0;
            if (block_len == 0) {
                continue;
            }

            const size_t packed_size = rhs_info->packed_size_ex(n, k, nr, kr, block_len);
            const size_t aligned_cursor = align_up(cursor, LM_GGML_KLEIDIAI_PACK_ALIGN);
...
static enum lm_ggml_status lm_ggml_backend_cpu_kleidiai_buffer_init_tensor(lm_ggml_backend_buffer_t buffer, struct lm_ggml_tensor * tensor) {
...
static void lm_ggml_backend_cpu_kleidiai_buffer_set_tensor(lm_ggml_backend_buffer_t buffer, struct lm_ggml_tensor * tensor,
...
static const char * lm_ggml_backend_cpu_kleidiai_buffer_type_get_name(lm_ggml_backend_buffer_type_t buft) {
    LM_GGML_UNUSED(buft);
    return "CPU_KLEIDIAI";
}

static lm_ggml_backend_buffer_t lm_ggml_backend_cpu_kleidiai_buffer_type_alloc_buffer(lm_ggml_backend_buffer_type_t buft, size_t size) {
    ...
    buffer->buft              = buft;
    buffer->iface.init_tensor = lm_ggml_backend_cpu_kleidiai_buffer_init_tensor;
    buffer->iface.set_tensor  = lm_ggml_backend_cpu_kleidiai_buffer_set_tensor;
    buffer->iface.get_tensor  = nullptr;
    buffer->iface.cpy_tensor  = nullptr;
    return buffer;
}

static size_t lm_ggml_backend_cpu_kleidiai_buffer_type_get_alignment(lm_ggml_backend_buffer_type_t buft) {
    LM_GGML_UNUSED(buft);
    return TENSOR_ALIGNMENT;
}

static size_t lm_ggml_backend_cpu_kleidiai_buffer_type_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * tensor) {
    LM_GGML_UNUSED(buft);

    if (tensor->type != LM_GGML_TYPE_Q4_0 && tensor->type != LM_GGML_TYPE_Q8_0) {
    ...
    for (int slot = 0; slot < slot_total; ++slot) {
        ...
        const lm_ggml_type rhs_type = kernels->rhs_type;
        const size_t block_len = rhs_type == LM_GGML_TYPE_Q4_0 ? QK4_0 :
                                 rhs_type == LM_GGML_TYPE_Q8_0 ? QK8_0 : 0;
        if (block_len == 0) {
            continue;
        }
    ...
    return std::max(cursor, lm_ggml_nbytes(tensor));
}
...
class extra_buffer_type : ggml::cpu::extra_buffer_type {
    bool supports_op(lm_ggml_backend_dev_t, const struct lm_ggml_tensor * op) override {
        ...
        if ((op->op == LM_GGML_OP_MUL_MAT || op->op == LM_GGML_OP_GET_ROWS) &&
            (op->src[0]->type == LM_GGML_TYPE_Q4_0 || op->src[0]->type == LM_GGML_TYPE_Q8_0) &&
            op->src[0]->buffer &&
            (lm_ggml_n_dims(op->src[0]) == 2) &&
            op->src[0]->buffer->buft == lm_ggml_backend_cpu_kleidiai_buffer_type() &&
            slot_total > 0) {
            if (op->src[0]->type == LM_GGML_TYPE_Q4_0 && ctx.kernels_q4 == nullptr) {
                return false;
            }
            if (op->src[0]->type == LM_GGML_TYPE_Q8_0 && ctx.kernels_q8 == nullptr) {
                return false;
            }
            if (op->src[1]->buffer && !lm_ggml_backend_buft_is_host(op->src[1]->buffer->buft)) {
                return false;
            }
            if ((op->src[1]->type == LM_GGML_TYPE_F32 || op->src[1]->type == LM_GGML_TYPE_I32) &&
                lm_ggml_ne(op->src[1], 3) == 1) {
                return true;
            }
        }
        return false;
    ...
    ggml::cpu::tensor_traits * get_tensor_traits(const struct lm_ggml_tensor * op) override {
        if (op->op == LM_GGML_OP_MUL_MAT || op->op == LM_GGML_OP_GET_ROWS) {
            if (op->src[0]->buffer && op->src[0]->buffer->buft == lm_ggml_backend_cpu_kleidiai_buffer_type()) {
...
lm_ggml_backend_buffer_type_t lm_ggml_backend_cpu_kleidiai_buffer_type(void) {
    static ggml::cpu::kleidiai::extra_buffer_type ctx;
    static struct lm_ggml_backend_buffer_type lm_ggml_backend_cpu_buffer_type_kleidiai = {
        /* .iface    = */ {
                           /* .get_name         = */ lm_ggml_backend_cpu_kleidiai_buffer_type_get_name,
                           /* .alloc_buffer     = */ lm_ggml_backend_cpu_kleidiai_buffer_type_alloc_buffer,
                           /* .get_alignment    = */ lm_ggml_backend_cpu_kleidiai_buffer_type_get_alignment,
                           /* .get_max_size     = */ nullptr,  // defaults to SIZE_MAX
                           /* .get_alloc_size   = */ lm_ggml_backend_cpu_kleidiai_buffer_type_get_alloc_size,
                           /* .is_host          = */ nullptr,
                           },
    ...
    return &lm_ggml_backend_cpu_buffer_type_kleidiai;
}

 ...
 *
 * Implements LM_GGML_TYPE_TURBO2_0 (2-bit), LM_GGML_TYPE_TURBO3_0 (3-bit) and
 * LM_GGML_TYPE_TURBO4_0 (4-bit) for use as --cache-type-k turboN in llama-server.
 */
...
/* Forward declarations for LM_GGML_API symbols defined in this file (satisfies
 * -Wmissing-prototypes under upstream CI's -Werror policy). */
LM_GGML_API void turbo_cpu_fwht_inverse(float * x, int group_size);

/* Global: WHT group size for CPU quantize path (set by CPU SET_ROWS handler) */
LM_GGML_API int turbo3_cpu_wht_group_size = 0;
...
void quantize_row_turbo3_0_ref(const float * LM_GGML_RESTRICT x, block_turbo3_0 * LM_GGML_RESTRICT y, int64_t k) {
    ...
    for (int g = 0; g < n_groups; g++) {
        ...
        for (int b = 0; b < blocks_per_group; b++) {
            grp_dst[b].norm = LM_GGML_FP32_TO_FP16(corrected);
        }
    }
}
...
void quantize_row_turbo2_0_ref(const float * LM_GGML_RESTRICT x, block_turbo2_0 * LM_GGML_RESTRICT y, int64_t k) {
...
void dequantize_row_turbo2_0(const block_turbo2_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TURBO2 == 0);
...
size_t quantize_turbo2_0(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst,
                         int64_t nrows, int64_t n_per_row, const float * imatrix) {
    LM_GGML_UNUSED(imatrix);
...
void quantize_row_turbo4_0_ref(const float * LM_GGML_RESTRICT x, block_turbo4_0 * LM_GGML_RESTRICT y, int64_t k) {
    ...
    for (int block = 0; block < nb; block++) {
        ...
        y[block].norm = LM_GGML_FP32_TO_FP16(corrected_norm);
#else
        ...
        y[block].rnorm = LM_GGML_FP32_TO_FP16(0.0f);
#else
...
void dequantize_row_turbo4_0(const block_turbo4_0 * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    ...
    for (int block = 0; block < nb; block++) {
        ...
        * The inverse WHT is applied to the attention output via LM_GGML_OP_TURBO_WHT (direction=1) in the graph. 
        */
    }
#else
...
void quantize_row_tq3_1s_ref(const float * LM_GGML_RESTRICT x, block_tq3_1s * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TQ3_0 == 0);
    ...
    for (int block = 0; block < nb; block++) {
        ...
        blk->d0 = LM_GGML_FP32_TO_FP16(best_d0);
        blk->d1 = LM_GGML_FP32_TO_FP16(best_d1);
...
void dequantize_row_tq3_1s(const block_tq3_1s * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TQ3_0 == 0);
    const int nb = k / QK_TQ3_0;

    for (int blk_i = 0; blk_i < nb; blk_i++) {
        float d0 = LM_GGML_FP16_TO_FP32(x[blk_i].d0);
        float d1 = LM_GGML_FP16_TO_FP32(x[blk_i].d1);
...
size_t quantize_tq3_1s(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst,
                        int64_t nrows, int64_t n_per_row, const float * imatrix) {
    LM_GGML_UNUSED(imatrix);
...
void quantize_row_tq4_1s_ref(const float * LM_GGML_RESTRICT x, block_tq4_1s * LM_GGML_RESTRICT y, int64_t k) {
...
void dequantize_row_tq4_1s(const block_tq4_1s * LM_GGML_RESTRICT x, float * LM_GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TQ4_1S == 0);
    const int nb = k / QK_TQ4_1S;
...
size_t quantize_tq4_1s(const float * LM_GGML_RESTRICT src, void * LM_GGML_RESTRICT dst,
                        int64_t nrows, int64_t n_per_row, const float * imatrix) {
    LM_GGML_UNUSED(imatrix);

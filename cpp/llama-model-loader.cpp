...
namespace GGUFMeta {
    ...
    llama_model_loader::get_arr_n(const std::string & key, T & result, bool required) {
        const int kid = lm_gguf_find_key(metadata, key.c_str());
    ...
    bool llama_model_loader::get_arr(const std::string & key, std::vector<T> & result, bool required) {
        const lm_gguf_context * ctx = metadata;
    ...
    bool llama_model_loader::get_key_or_arr(enum llm_kv kid, uint32_t & result, bool required) {
        ...
        const int id = lm_gguf_find_key(metadata, key.c_str());
        ...
        if (lm_gguf_get_kv_type(metadata, id) == LM_GGUF_TYPE_ARRAY) {
            if (required) {
...
llama_model_loader::llama_model_loader(
    ...
    if (!fname.empty()) {
        ...
        metadata_ptr.reset(lm_gguf_init_from_file(fname.c_str(), params));
    ...
    n_kv      = lm_gguf_get_n_kv(metadata);
    ...
    // TODO: make optional
    {
        ...
        switch (type_max) {
            ...
            case LM_GGML_TYPE_TQ3_1S:  ftype = LLAMA_FTYPE_MOSTLY_TQ3_1S;  break;
            case LM_GGML_TYPE_TQ4_1S:  ftype = LLAMA_FTYPE_MOSTLY_TQ4_1S;  break;
            ...
            case LM_GGML_TYPE_NVFP4:   ftype = LLAMA_FTYPE_MOSTLY_NVFP4;   break;
            case LM_GGML_TYPE_Q1_0:    ftype = LLAMA_FTYPE_MOSTLY_Q1_0;    break;
            default:
                {
        ...
        for (int i = 0; i < n_kv; i++) {
            const char * name           = lm_gguf_get_key(metadata, i);
            const enum lm_gguf_type type   = lm_gguf_get_kv_type(metadata, i);
            const std::string type_name =
                type == LM_GGUF_TYPE_ARRAY
                ? format("%s[%s,%zu]", lm_gguf_type_name(type), lm_gguf_type_name(lm_gguf_get_arr_type(metadata, i)), lm_gguf_get_arr_n(metadata, i))
                : lm_gguf_type_name(type);

            std::string value          = lm_gguf_kv_to_str(metadata, i);
        ...
        case LM_GGML_OP_ADD_ID:
            {
                const int n_expert_used = hparams.n_expert_used;
                LM_GGML_ASSERT(n_expert_used > 0);
                lm_ggml_tensor * a = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, w->ne[0], n_expert_used, 512);
                lm_ggml_tensor * c = lm_ggml_new_tensor_2d(ctx, LM_GGML_TYPE_I32, n_expert_used, 512);
                op_tensor = lm_ggml_add_id(ctx, a, w, c);
            } break;
        case LM_GGML_OP_MUL:
            {
                lm_ggml_tensor * a = lm_ggml_new_tensor_4d(ctx, LM_GGML_TYPE_F32, w->ne[0], w->ne[1], w->ne[2], w->ne[3]);
        ...
        case LM_GGML_OP_SSM_CONV:
            {
                ...
                lm_ggml_tensor * conv_x = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, w->ne[0] - 1 + n_seq_tokens, w->ne[1], n_seqs);
        ...
        case LM_GGML_OP_SSM_SCAN:
            {
                ...
                const int64_t n_seq_tokens = 512;
                const int64_t n_seqs       = 3;
                lm_ggml_tensor * s   = lm_ggml_new_tensor_4d(ctx, LM_GGML_TYPE_F32, d_state, head_dim, n_head, n_seqs);
                lm_ggml_tensor * x   = lm_ggml_new_tensor_4d(ctx, LM_GGML_TYPE_F32, head_dim, n_head, n_seq_tokens, n_seqs);
                lm_ggml_tensor * dt  = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, n_head, n_seq_tokens, n_seqs);
        ...
        case LM_GGML_OP_RWKV_WKV6:
            {
                ...
                lm_ggml_tensor  * k = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, S, H, n_tokens);
                lm_ggml_tensor  * v = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, S, H, n_tokens);
                lm_ggml_tensor  * r = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, S, H, n_tokens);
                lm_ggml_tensor  * tf = w;
                lm_ggml_tensor  * td = lm_ggml_new_tensor_3d(ctx, LM_GGML_TYPE_F32, S, H, n_tokens);
        ...
        case LM_GGML_OP_IM2COL:
            {
                ...
                lm_ggml_tensor * b = lm_ggml_new_tensor_4d(ctx, LM_GGML_TYPE_F32, n_embd_inp, w->ne[1], 1, 1);
                op_tensor = lm_ggml_im2col(ctx, w, b, 1, 0, 0, 0, 1, 0, false, LM_GGML_TYPE_F16);
            } break;
        case LM_GGML_OP_SCALE:
            {
                op_tensor = lm_ggml_scale(ctx, w, 1.0f);
            } break;
        default:
...
static lm_ggml_backend_buffer_type_t select_weight_buft(const llama_hparams & hparams, lm_ggml_tensor * tensor, lm_ggml_op op, const buft_list_t * buft_list) {
...
struct lm_ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto buft_for_tensor = [&](lm_ggml_tensor * t_meta) -> lm_ggml_backend_buffer_type_t {
        if (!t_meta) {
        ...
        // skip unused tensors
        if (info.op == LM_GGML_OP_NONE || (flags & TENSOR_SKIP)) {
            const size_t nbytes = lm_ggml_nbytes(t_meta);
        ...
        switch (info.layer) {
            ...
            case LLM_TENSOR_LAYER_REPEATING:
                LM_GGML_ASSERT(buft_list_layer != nullptr);
        ...
        if (tensor_buft_overrides) {
            ...
            for (const auto * overrides = tensor_buft_overrides; overrides->pattern != nullptr; ++overrides) {
                ...
                if (std::regex_search(tensor_name, pattern)) {
                    if (overrides->buft == lm_ggml_backend_cpu_buffer_type()) {
        ...
        if (use_mmap && buft_dev && buft == lm_ggml_backend_dev_host_buffer_type(buft_dev)) {
        ...
        if (buft != buft_list->front().second) {
            if (n_tensors_moved == 0) {
                ...
                first_tensor_moved_type_name = lm_ggml_type_name(t_meta->type);
    ...
    if (files.empty()) {
        ...
        const int64_t tid = lm_gguf_find_tensor(metadata, tn.str().c_str());
        if (tid != -1) {
            type = lm_gguf_get_tensor_type(metadata, tid);
        }
        ...
        lm_ggml_tensor t_meta;
        memset(&t_meta, 0, sizeof(lm_ggml_tensor));
        t_meta.type = type;
        for (size_t dim = 0; dim < LM_GGML_MAX_DIMS; dim++) {
            t_meta.ne[dim] = dim < ne.size() ? ne.begin()[dim] : 1;
            LM_GGML_ASSERT(t_meta.ne[dim] >= 1);
            t_meta.nb[dim] = dim == 0 ? lm_ggml_type_size(type) : t_meta.ne[dim-1]*t_meta.nb[dim-1];
            LM_GGML_ASSERT(t_meta.nb[dim] >= 1);
        }
        lm_ggml_set_name(&t_meta, tn.str().c_str());

        lm_ggml_backend_buffer_type_t buft = buft_for_tensor(&t_meta);
        LM_GGML_ASSERT(buft != nullptr);
        lm_ggml_context * ctx = ctx_for_buft(buft);
        lm_ggml_tensor * ret = lm_ggml_dup_tensor(ctx, &t_meta);
        lm_ggml_set_name(ret, tn.str().c_str());
        return ret;
    }

    lm_ggml_tensor * t_meta = get_tensor_meta(tn.str().c_str());
    lm_ggml_backend_buffer_type_t buft = buft_for_tensor(t_meta);
    if (buft == nullptr) {
        return nullptr; // return type is lm_ggml_tensor *
    }
    lm_ggml_context * ctx = ctx_for_buft(buft);
    ...
    const struct lm_ggml_tensor * cur = check_tensor_dims(tn.str(), ne, !(flags & TENSOR_NOT_REQUIRED));
            ...
            if (lm_ggml_backend_buffer_is_host(cur->buffer)) {
                ...
                if (upload_backend) {
                    ...
                    while (bytes_read < read_end - read_start) {
                        ...
                        lm_ggml_backend_tensor_set_async(upload_backend, cur,
